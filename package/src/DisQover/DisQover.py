...
import itertools
from ipywidgets import interact, interactive,HBox,Layout,VBox,interactive_output
...
class QRW_simulation:
    ...
    def conv2d(self,tensor: np.ndarray,n: int)-> np.ndarray:
        ...
        if bit == "00":
            tensor = np.concatenate((tensor[:,1:],np.zeros((tensor.shape[0],1))),axis=1)
            tensor = np.concatenate((np.zeros((1,tensor.shape[1])),tensor[:-1,:]),axis=0)       
        if bit == "01":
            tensor = np.concatenate((tensor[:,1:],np.zeros((tensor.shape[0],1))),axis=1)
            tensor = np.concatenate((tensor[1:,:],np.zeros((1,tensor.shape[1]))),axis=0)
        if bit == "10":
            tensor = np.concatenate((np.zeros((tensor.shape[0],1)),tensor[:,:-1]),axis=1)
            tensor = np.concatenate((np.zeros((1,tensor.shape[1])),tensor[:-1,:]),axis=0)
        if bit == "11":
            tensor = np.concatenate((np.zeros((tensor.shape[0],1)),tensor[:,:-1]),axis=1)
            tensor = np.concatenate((tensor[1:,:],np.zeros((1,tensor.shape[1]))),axis=0)
    ...
    def simulate(self, coin_state: List[np.ndarray],U: List[np.ndarray],init_amp: np.ndarray,steps: int,return_amp: bool=False) -> np.ndarray:
        ...
        tensor = np.tensordot(np.array(coin_vec).reshape(len(coin_vec),1),init_amp.reshape((1,)+init_amp.shape),axes=1)
        ...
        for step in range(steps):
            ...
            if n == 2:
                ...
                shift_tensors = [self.conv2d(tensor[i],i).reshape((1,)+tensor.shape[1:]) for i in range(tensor.shape[0])]
                tensor = np.concatenate(shift_tensors,axis=0)
            else:
                tensor = Un@tensor
                tensor = np.vstack((self.conv(tensor[0,:],1),self.conv(tensor[1,:],-1))) 
            
        
        if return_amp == True:
            return tensor
        else:
            prob = np.tensordot(np.ones((1,tensor.shape[0])),(tensor*np.conjugate(tensor)).real,axes=1).reshape(init_amp.shape)
            return prob
...
class CRW_simulation:
    ...
    def simulate(self, init_prob: np.ndarray, steps: int, plot: bool=False)-> np.ndarray:
        ...
        coin_probs = np.array([self.coin_probs[0],self.coin_probs[1]])/np.sum(self.coin_probs)
        ...
        if plot:
            ...
            cmap1D = matplotlib.colors.LinearSegmentedColormap.from_list("", ["indigo","blueviolet","violet","magenta"])
            norm = plt.Normalize(vmin=min(prob), vmax=max(prob))  # Normalize the data for colormap
            ...
            if N % 2 ==0: 
                ...
                plt.bar(range(-int((N-1)/2),int((N-1)/2)+1),prob,color=colors)

            plt.title(f"Classical Random Walks \n coin = [{coin_probs[0]:.3f},{coin_probs[1]:.3f}], $\\quad$ Steps = {steps}",fontsize=15)
...
class QRW_qiskit_simulation:
    ...
    def circuit(self,steps: int,theta: float,lamb: float,coin_state: np.ndarray,draw: bool=True)-> QuantumCircuit:
    ...
    def memory_circuit(self,coin_angles: np.ndarray, memory_angle: float, draw: bool=True)-> QuantumCircuit:
    ...
    def simulate(self,steps: int,theta: float,lamb: float,coin_state: np.ndarray,shots: int=10000)-> np.ndarray:
    ...
    def memory_simulate(self,coin_angles: np.ndarray, memory_angle: float, shots: int=10000)-> np.ndarray:
...
class harmonic_QRW_simulation:
    ...
    def simulate(self, coin_state: np.ndarray,U: List[np.ndarray],init_amp: np.ndarray,
                 steps: int, period: int, return_amp: bool=False) -> np.ndarray:
        ...
        for step in range(steps):
            ...
            tensor = np.vstack((self.simulator.conv(tensor[0,:],1),self.simulator.conv(tensor[1,:],-1))) 
...
class qutrit_QRW_simulation:
    ...
    def simulate(self, coin_state: np.ndarray, U: np.ndarray, init_amp: np.ndarray, steps: int, return_amp: bool=False) -> np.ndarray:
        ...
        for step in range(steps):
            ...
            tensor = np.vstack((self.simulator.conv(tensor[0,:],1),tensor[1,:],self.simulator.conv(tensor[2,:],-1))) 
...
class QRW_visualization:
    ...
    def __init__(self,init_amp: np.ndarray, max_steps: int ,dim: int, model: str="basic", plot_type: str="time_slice"):
        ...
        if len(init_amp.shape) != dim:
            raise ValueError("the dimension of init_amp does not match the specify dim.")
        if dim not in [1,2]:
        ...
        if dim == 1:
            self.init_amp = np.hstack((np.zeros(max_steps),init_amp,np.zeros(max_steps)))
        else:
    ...
    def plot(self)-> None:
        ...
        sns.set_style("darkgrid")
        cmap = matplotlib.colors.LinearSegmentedColormap.from_list("", ["k","rebeccapurple","blueviolet","mediumorchid","magenta"])
        ...
        if self.dim == 1:
            if self.plot_type == "time_slice": #1D QRW time_slice plot
                ...
                if self.model == "basic":
                    ...
                    def mono_time_slice_1D(steps, theta, lamb, a, phi):
                        ...
                        result = self.simulator.simulate([coin_state], [U], self.init_amp, steps)
                        ...
                        if len(init_amp) %2 !=0:
                            plt.bar(range(-self.bound, self.bound + 1), result, color=colors)
                        else:
                            plt.bar(range(-self.bound, self.bound), result, color=colors)
                    
                        plt.xticks(rotation=60)
                        plt.xlabel("Positions", fontsize=15)
                        plt.ylabel("Probability", fontsize=15)
                        plt.title(f"Quantum Random Walks $\\quad$ Steps = {steps} \n θ = {round(theta / np.pi, 3)}π $\\quad$ λ = {round(lamb / np.pi, 3)}π $\\quad$ coin state = {np.round(coin_state, 3)}", fontsize=15)
                        plt.show()

                
                    widget = interactive(mono_time_slice_1D, steps = widgets.IntSlider(value=0,min=0,max=max_steps,step=1,description='Steps'),
                         theta = widgets.FloatSlider(value=np.pi/2,min=0,max=np.pi,step=0.05*np.pi,description='θ'),
                        lamb = widgets.FloatSlider(value=np.pi,min=0,max=2*np.pi,step=0.05*np.pi,description='λ'),
                        a = widgets.FloatSlider(value=0,min=0,max=1,step=0.05,description='a'),
                        phi = widgets.FloatSlider(value=0,min=0,max=2*np.pi,step=0.05*np.pi,description='ϕ'))
                ...
                if self.model == "harmonic": #1D harmonic QRW time_slice plot
                    ...
                    def harmonic_time_slice_1D(steps, theta1, lamb1, theta2, lamb2, a, phi, period):
                       
                        U1 = np.array([[np.cos(theta1 / 2), -np.exp(lamb1 * 1j) * np.sin(theta1 / 2)],
                                    [np.sin(theta1 / 2), np.exp(lamb1 * 1j) * np.cos(theta1 / 2)]])
                        U2 = np.array([[np.cos(theta2 / 2), -np.exp(lamb2 * 1j) * np.sin(theta2 / 2)],
                        ...
                        # Run the QRW simulation
                        result = self.simulator.simulate(coin_state, [U1,U2], self.init_amp, steps, period)
                        ...
                        plt.xlabel("Positions", fontsize=15)
                        plt.ylabel("Probability", fontsize=15)
                        plt.title(f"Harmonic Quantum Random Walks $\\quad$ Steps = {steps} \n θ1 = {round(theta1 / np.pi, 3)}π $\\quad$ λ1 = {round(lamb1 / np.pi, 3)}π $\\quad$ θ2 = {round(theta2 / np.pi, 3)}π $\\quad$ λ2 = {round(lamb2 / np.pi, 3)}π $\\quad$ coin state = {np.round(coin_state, 3)} $\\quad$ period = {period}", fontsize=15)
                        plt.show()
                
                    widget = interactive(harmonic_time_slice_1D, steps = widgets.IntSlider(value=0,min=0,max=max_steps,step=1,description='Steps'),
                                         theta1 = widgets.FloatSlider(value=np.pi/2,min=0,max=np.pi,step=0.05*np.pi,description='θ1'),
                                         lamb1 = widgets.FloatSlider(value=np.pi,min=0,max=2*np.pi,step=0.05*np.pi,description='λ1'),
                                         theta2 = widgets.FloatSlider(value=np.pi/2,min=0,max=np.pi,step=0.05*np.pi,description='θ2'),
                                         lamb2 = widgets.FloatSlider(value=np.pi,min=0,max=2*np.pi,step=0.05*np.pi,description='λ2'),
                                         a = widgets.FloatSlider(value=0,min=0,max=1,step=0.05,description='a'),
                                         phi = widgets.FloatSlider(value=0,min=0,max=2*np.pi,step=0.05*np.pi,description='ϕ'),
                                        period = widgets.IntSlider(value=1,min=1,max=max_steps+1,step=1,description='period'))
                ...
                if self.model == "qutrit": # 1d qutrit QRW time_slice plot
                    ...
                    Steps_slid = widgets.IntSlider(value=0,min=0,max=max_steps,step=1,description='Steps')
                    theta_slid = widgets.FloatSlider(value=np.pi/2,min=0,max=np.pi,step=0.05*np.pi,description='θ')
                    lamb_slid = widgets.FloatSlider(value=np.pi,min=0,max=2*np.pi,step=0.05*np.pi,description='λ')
                    a_slid = widgets.FloatSlider(value=0,min=0,max=1,step=0.05,description='a')
                    b_slid = widgets.FloatSlider(value=0,min=0,max=1,step=0.05,description='b')
                    phi1_slid = widgets.FloatSlider(value=0,min=0,max=2*np.pi,step=0.05*np.pi,description='ϕ1')
                    phi2_slid = widgets.FloatSlider(value=0,min=0,max=2*np.pi,step=0.05*np.pi,description='ϕ2')
                    ...
                    def qutrit_time_slice_1D(steps, theta, lamb, a, b, phi1, phi2):
                        ...
                        # Define the coin operator U based on theta and lambda
                        U = np.array([[np.exp(lamb*1j)*np.cos(theta/2)**2,1/2**0.5*np.sin(theta),np.exp(-lamb*1j)*np.sin(theta/2)**2]
                                  ,[-1/2**0.5*np.exp(lamb*1j)*np.sin(theta),np.cos(theta),1/2**0.5*np.exp(-lamb*1j)*np.sin(theta)]
                                  ,[np.exp(lamb*1j)*np.sin(theta/2)**2,-1/2**0.5*np.sin(theta),np.exp(-lamb*1j)*np.cos(theta/2)**2]])

                        # Define the coin state
                        coin_state = np.array([a, b*np.exp(1j*phi1), (1-a**2-b**2)**0.5*np.exp(1j*phi2)])
                        ...
                        plt.xlabel("Positions", fontsize=15)
                        plt.ylabel("Probability", fontsize=15)
                        plt.title(f"Quantum Random Walks with Qutrit $\\quad$ Steps = {steps} \n θ = {round(theta / np.pi, 3)}π $\\quad$ λ = {round(lamb / np.pi, 3)}π $\\quad$ coin state = {np.round(coin_state, 3)}", fontsize=15)
                        plt.show()

        
                    widget = interactive(qutrit_time_slice_1D, steps=Steps_slid, theta=theta_slid, lamb=lamb_slid, a=a_slid, b=b_slid, phi1= phi1_slid, phi2 = phi2_slid)
                ...
                if self.model == "basic": # 1D basic QRW timeline plot
                    def mono_timeline_1D(steps,theta,lamb,a,phi):
                        ...
                        evolution = np.array([self.simulator.simulate([coin_state],[U],init_amp,s) for s in range(steps)])
                        evolution = np.array([self.normalize(evolution[i,:]) for i in range(steps)])
                        ...
                        if len(init_amp) %2 !=0: 
                            xlabels=[f"{int(x)}" for x in np.arange(-self.bound,self.bound+1,5)]
                        else:
                            xlabels=[f"{int(x)}" for x in np.arange(-self.bound,self.bound,5)]
                        ylabels=[f"{int(5*i)}" for i in range(steps//5+1)]
    
                        plt.xticks(np.arange(xmin,xmax+1,5),labels=xlabels,fontsize=15)
                        plt.yticks(list(range(0,5*(steps//5)+1,5)),labels=ylabels,fontsize=15)

                        plt.xlabel("Positions",fontsize=15)
                        plt.ylabel("Steps",fontsize=15)
                        plt.title(f"Quantum Random Walks \n θ = {round(theta/np.pi,3)}π $\\quad$ λ = {round(lamb/np.pi,3)}π $\\quad$ coin state = {np.round(coin_state,3)}",fontsize=15)
                        plt.show()
    
                    widget = interactive(mono_timeline_1D, steps = widgets.IntSlider(value=0,min=0,max=max_steps,step=1,description='Steps'),
                             theta = widgets.FloatSlider(value=np.pi/2,min=0,max=2*np.pi,step=0.05*np.pi,description='θ'),
                             lamb = widgets.FloatSlider(value=np.pi,min=0,max=2*np.pi,step=0.05*np.pi,description='λ'),
                             a = widgets.FloatSlider(value=0,min=0,max=1,step=0.05,description='a'),
                             phi = widgets.FloatSlider(value=0,min=0,max=2*np.pi,step=0.05*np.pi,description='ϕ'))
                ...
                if self.model == "harmonic":# 1D harmonic QRW timeline plot
                    def harmonic_timeline_1D(steps,theta1,lamb1,theta2,lamb2,a,phi,period):
                        ...
                        evolution = np.array([self.simulator.simulate(coin_state,[U1,U2],init_amp,s,period) for s in range(steps)])
                        evolution = np.array([self.normalize(evolution[i,:]) for i in range(steps)])
                        ...
                        plt.xlabel("Positions",fontsize=15)
                        plt.ylabel("Steps",fontsize=15)
                        plt.title(f"Harmonic Quantum Random Walks \n θ1 = {round(theta1/np.pi,3)}π $\\quad$ λ1 = {round(lamb1/np.pi,3)}π $\\quad$ θ2 = {round(theta2/np.pi,3)}π $\\quad$ λ2 = {round(lamb2/np.pi,3)}π $\\quad$ coin state = {np.round(coin_state,3)} $\\quad$ period = {period}",fontsize=15)
                        plt.show()
    

                    widget = interactive(harmonic_timeline_1D, steps = widgets.IntSlider(value=0,min=0,max=max_steps,step=1,description='Steps'),
                             theta1 = widgets.FloatSlider(value=np.pi/2,min=0,max=2*np.pi,step=0.05*np.pi,description='θ1'),
                             lamb1 = widgets.FloatSlider(value=np.pi,min=0,max=2*np.pi,step=0.05*np.pi,description='λ1'),
                             theta2 = widgets.FloatSlider(value=np.pi/2,min=0,max=2*np.pi,step=0.05*np.pi,description='θ2'),
                             lamb2 = widgets.FloatSlider(value=np.pi,min=0,max=2*np.pi,step=0.05*np.pi,description='λ2'),
                             a = widgets.FloatSlider(value=0,min=0,max=1,step=0.05,description='a'),
                             phi = widgets.FloatSlider(value=0,min=0,max=2*np.pi,step=0.05*np.pi,description='ϕ'),
                             period = widgets.IntSlider(value=1,min=1,max=max_steps+1,step=1,description='period'))
                ...
                if self.model == "qutrit": # 1D qutrit QRW timeline plot
                    ...
                    def qutrit_timeline_1D(steps,theta,lamb,a,b,phi1,phi2):
                        ...
                        plt.xlabel("Positions",fontsize=15)
                        plt.ylabel("Steps",fontsize=15)
                        plt.title(f"Quantum Random Walks with Qutrit \n θ = {round(theta/np.pi,3)}π $\\quad$ λ = {round(lamb/np.pi,3)}π $\\quad$ coin state = {np.round(coin_state,3)}",fontsize=15)
                        plt.show()
                         

                    widget = interactive(qutrit_timeline_1D, steps=Steps_slid, theta=theta_slid, lamb=lamb_slid, a=a_slid, b=b_slid, phi1= phi1_slid, phi2 = phi2_slid)
            ...
            def time_slice_2D(steps,theta1,lamb1,theta2,lamb2,a1,phi1,a2,phi2):
                ...
                coin_state = np.array([[a1,(1-a1**2)**0.5*np.exp(1j*phi1)],[a2,(1-a2**2)**0.5*np.exp(1j*phi2)]])
                ...
                if steps ==0:
                    ...
                    ax = sns.heatmap((ppp-np.min(ppp))/(np.max(ppp)-np.min(ppp)),cmap=cmap)
        
                if len(init_amp) % 2 !=0:
                    ax.set_xticks(range(0,len(init_amp),5),labels=[f"{i}" for i in range(-self.bound,self.bound+1,5)])
                    ax.set_yticks(range(0,len(init_amp),5),labels=[f"{i}" for i in range(self.bound,-self.bound-1,-5)])
                else:
                    ax.set_xticks(range(0,len(init_amp),5),labels=[f"{i}" for i in range(-self.bound,self.bound,5)])
                    ax.set_yticks(range(0,len(init_amp),5),labels=[f"{i}" for i in range(self.bound,-self.bound,-5)])
                plt.show()    
            
            widget = interactive(time_slice_2D, steps = widgets.IntSlider(value=0,min=0,max=max_steps,step=1,description='Steps'),
                 theta1 = widgets.FloatSlider(value=np.pi/2,min=0,max=2*np.pi,step=0.05*np.pi,description='θ1'),
                lamb1 = widgets.FloatSlider(value=np.pi,min=0,max=2*np.pi,step=0.05*np.pi,description='λ1'),
                theta2 = widgets.FloatSlider(value=np.pi/2,min=0,max=2*np.pi,step=0.05*np.pi,description='θ2'),
                lamb2 = widgets.FloatSlider(value=np.pi,min=0,max=2*np.pi,step=0.05*np.pi,description='λ2'),
                 a1 = widgets.FloatSlider(value=0,min=0,max=1,step=0.05,description='a1'),
                phi1 = widgets.FloatSlider(value=0,min=0,max=2*np.pi,step=0.05*np.pi,description='ϕ1'),
                a2 = widgets.FloatSlider(value=0,min=0,max=1,step=0.05,description='a2'),
                phi2 = widgets.FloatSlider(value=0,min=0,max=2*np.pi,step=0.05*np.pi,description='ϕ2'))
...
class QRW_optimization:
    ...
    def objective_mono(self,x: np.ndarray,*args)-> float:
        ...
        init_amp = np.array([1 if i == int(start) else 0 for i in range(-self.max_steps,self.max_steps+1)])
    ...
    def objective_harmonic(self,x: np.ndarray,*args)-> float:
        ...
        fit_prob = self.simulator.simulate(coin_state,[U1,U2],init_amp,steps,period)
    ...
    def optimize(self,rounds: int=500,plot: bool=True)-> tuple[dict[str, float], np.ndarray]:
        ...
        if self.model == "basic":
            ...
            bounds = [(-self.max_steps,self.max_steps+0.5),(0,self.max_steps+0.5),(0,np.pi),(0,2*np.pi),(0,1),(0,2*np.pi)]
            ...
            for i in range(rounds):
                ...
                x0 = np.array([self.max_steps+0.5,np.pi,2*np.pi,1,2*np.pi])*np.random.rand(5)
                x0 = np.hstack((start0,x0))
                opt = minimize(self.objective_mono,x0,args=(self.end_prob),method="SLSQP",bounds=bounds,tol=1e-8,
            ...
            U_sol = np.array([[np.cos(theta1/2),-np.exp(lamb1*1j)*np.sin(theta1/2)],
                    [np.sin(theta1/2),np.exp(lamb1*1j)*np.cos(theta1/2)]])
    
            init_amp = np.array([1 if i == int(start1) else 0 for i in range(-self.max_steps,self.max_steps+1)])
            ...
            n = max(len(np.where(self.end_prob !=0)[0]),len(np.where(fit_prob !=0)[0]))
    
            best_loss = np.min(loss)/n

            print(f"MSE: {best_loss}, std : {np.std(loss)/n}, x0: {int(start1)}, steps: {steps1}, θ: {theta1:.3f}, λ: {lamb1:.3f}, a: {a1:.3f}, ϕ: {phi1:.3f}")

            output = {"x0":int(start1),"steps":steps1,"θ":theta1,"λ":lamb1,"a":a1,"ϕ":phi1}
            ...
            if plot:
                ...
                ax.bar(range(-self.max_steps,self.max_steps+1),fit_prob,label="fitter",width=0.7*(0.5+0.01*self.max_steps),align="edge",color="rebeccapurple")
                ax.bar(range(-self.max_steps,self.max_steps+1),self.end_prob,label="target",width=-0.5*(0.5+0.01*self.max_steps),align="edge",color="magenta")
                ax.bar([0],[0],label=f"MSE = {1000*best_loss:.5f}e-3",color="white",alpha=0.01)
                ax.set_xlabel("Positions",fontsize=15)
                ax.set_ylabel("Probability",fontsize=15)
                ax.set_title(f"Quantum Random Walks Fitter \n $x_{0}$={int(start1)},  Steps={steps1},  θ={round(theta1,3)},  λ={round(lamb1,3)},  a={round(a1,3)},  ϕ={round(phi1,3)}",fontsize=15)
                ax.legend(fontsize=15)
                plt.show()
        
        else: #harmonic QRW model

            bounds = [(-self.max_steps,self.max_steps+0.5),(0,self.max_steps+0.5),(0,np.pi),(0,2*np.pi),(0,np.pi),(0,2*np.pi),(0,1),(0,2*np.pi),(1,self.max_steps+2)]
            ...
            for i in range(rounds):
                ...
                x0 = np.array([self.max_steps+0.5,np.pi,2*np.pi,np.pi,2*np.pi,1,2*np.pi])*np.random.rand(7)
                x0 = np.hstack((start0,x0,period0))
                opt = minimize(self.objective_harmonic,x0,args=(self.end_prob),method="SLSQP",bounds=bounds,tol=1e-8,
                    options={"maxiter": 200000})
            ...
            U_sol1 = np.array([[np.cos(theta1/2),-np.exp(lamb1*1j)*np.sin(theta1/2)],
                    [np.sin(theta1/2),np.exp(lamb1*1j)*np.cos(theta1/2)]])
            U_sol2 = np.array([[np.cos(theta2/2),-np.exp(lamb2*1j)*np.sin(theta2/2)],
                    [np.sin(theta2/2),np.exp(lamb2*1j)*np.cos(theta2/2)]])
            ...
            fit_prob = self.simulator.simulate(coin_state1,[U_sol1,U_sol2],init_amp,steps1,int(period1))

            n = max(len(np.where(self.end_prob !=0)[0]),len(np.where(fit_prob !=0)[0]))
    
            best_loss = np.min(loss)/n
    
            print(f"MSE: {best_loss}, std : {np.std(loss)/n}, x0: {int(start1)}, steps: {steps1}, θ1: {theta1:.3f}, λ1: {lamb1:.3f}, θ2: {theta2:.3f}, λ2: {lamb2:.3f}, a: {a1:.3f}, ϕ: {phi1:.3f}, period: {int(period1)}")
    
            output = {"x0":int(start1),"steps":steps1,"θ1":theta1,"λ1":lamb1,"θ2":theta2,"λ2":lamb2,"a":a1,"ϕ":phi1,"period":int(period1)}
            ...
            if plot:
                ...
                ax.set_xlabel("Positions",fontsize=15)
                ax.set_ylabel("Probability",fontsize=15)
                ax.set_title(f"Harmonic Quantum Random Walks Fitter \n $x_{0}$={int(start1)},  Steps={steps1},  θ1={round(theta1,3)},  λ1={round(lamb1,3)},  a={round(a1,3)},  ϕ={round(phi1,3)}, θ2={round(theta2,3)},  λ2={round(lamb2,3)}, period={int(period1)}",fontsize=15)
                ax.legend(fontsize=15)
                plt.show()
